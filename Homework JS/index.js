...
const register = event => {
    ...
    if(family_name == ""){
        errorsArray.push("Невалидно фамилно име");
    }
    else if(family_name.length > 50){
        errorsArray.push("Невалидно фамилно име - трябва да е по-малко от 50 символа");
    }

    //Email
    let email = document.getElementById('email').value;
    if(email == ""){
        errorsArray.push("Невалиден e-mail");
    }
            ...
            for(var i=0; i<objects.length; i++){
                if(//objects[i]["name"] == user["FirstName"] + " " + user["LastName"] &&
                    ...
                    /* objects[i]["address"]["zipcode"] == user["Address"]["PostalCode"]*/){
                        ...
                        isUserIn = true;
                        document.getElementById('success').innerText = "";
                        document.getElementsByClassName('error')[0].innerText = "Потребител с това потребителско име вече съществува!";
                }
            }
        };
